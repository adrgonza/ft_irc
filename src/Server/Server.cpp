...
bool Server::handleClientInput(Client &caller, std::string message)
{
	...
	else
	{
		...
		{
			std::string send = ERR_PASSWDREQUIRED(caller.getNickname().c_str());
...
void Server::checkPassword(std::string body, Client &caller)
{
	...
	{
		caller.giveKey(true);
		char* send = RPL_MOTDSTART(caller.getNickname().c_str(), "Welcome to the TONY_WARRIORS Internet Relay Chat Network");
		caller.sendMessage(send);
	}
	else
	{
